...
| :--- | :--- | :--- | :--- |
| **Overall Benchmark Pass Rate** | **{report_data['summary']['pass_rate']}%** | 100.0% | Pass |
| **Grounded Answer Rate** | **{report_data['metrics']['grounded_answer_rate']}%** | > 92.0% | Pass |
| **Unsupported Answer / Hallucination Rate** | **{report_data['metrics']['unsupported_answer_rate']}%** | < 3.0% | Pass (Zero Hallucination) |
...
| :--- | :--- | :--- | :--- | :--- |
| **Pipeline Overhead (Demo Provider)** | Deterministic In-Memory | `{report_data['efficiency']['pipeline_overhead_ms']} ms` | `< 1.0 ms` | **Zero network hop**: Measures PDF/DOCX parsing, BM25 indexing, tokenization, and strict containment verification. |
...
- **Cache Hit Rate**: **`{cache_stats['hit_rate_pct']}%`** ({cache_stats['hits']} hits / {cache_stats['total_cached_items']} total cached items)
